...
static void serve_server_statssent(char *arg)
{
	const char *statssent = NULL;

	// first need to find out if we attempted or if we were successful
	statssent = xstrdup(arg);

	// the response from the client should always be 2 chars
	//  1st char: Attempted
	//  2nd char: Attempt Successful
#ifdef _WIN32
	if (strlen(statssent)==2)
	{
		HKEY hk;

		// this code is copied from Service.cpp, it sets the registry on the server
		if(!RegOpenKeyEx(HKEY_LOCAL_MACHINE,_T("Software\\CVS\\Pserver"),NULL,KEY_QUERY_VALUE|KEY_SET_VALUE,&hk))
		{
			__time64_t now;
			servtime_t lastAttempt;

			_time64(&now);
			lastAttempt.t=now;
			if (*statssent=='1')
			{
				TRACE(3,"server-stats-sent record the attempt to send");
				RegSetValueEx(hk,_T("LastAttemptedSend"),NULL,REG_BINARY,(LPBYTE)&lastAttempt.data,sizeof(lastAttempt.data));
			}
			if (*(statssent+1)=='1')
			{
				TRACE(3,"server-stats-sent record the send was succssful");
				RegSetValueEx(hk,_T("LastSend"),NULL,REG_BINARY,(LPBYTE)&lastAttempt.data,sizeof(lastAttempt.data));
			}
			RegCloseKey(hk);
		}
		else
			TRACE(3,"server-stats-sent cannot open Software\\CVS\\Pserver for write!");

	}
	else
		TRACE(3,"server-stats-sent argumet != 2 chars -- something is wrong!");
#endif
}


static void serve_server_statsok(char *arg)
{
#ifdef _WIN32
	servtime_t lastAttempt,lastSuccess;
	HKEY hk;
	DWORD dwSendStatistics=0, dwNeedToSend=0;
#else
	int dwSendStatistics=0, dwNeedToSend=0;
#endif
#ifdef _WIN32
	lastSuccess.t=0;
	lastAttempt.t=0;

	// this code is copied from Service.cpp
	if(!RegOpenKeyEx(HKEY_LOCAL_MACHINE,_T("Software\\CVS\\Pserver"),NULL,KEY_QUERY_VALUE|KEY_SET_VALUE,&hk))
	{
		DWORD dwTmp,dwType;
...
#endif
	buf_output0(buf_to_net,(dwSendStatistics)?"1":"0");
	buf_output0(buf_to_net,(dwNeedToSend)?"1":"0");
	buf_output0(buf_to_net,"\n");
	buf_flush(buf_to_net,0);
}

bool SendStatistics(cvs::string& url)
{
#ifdef _WIN32
	...
	HKEY hInstallerKey,hInstallerKeyU,hServerKey;
	...
	if(RegOpenKeyEx(HKEY_CURRENT_USER,_T("Software\\March Hare Software Ltd\\Keys"),0,KEY_READ,&hInstallerKeyU))
	{
		hInstallerKeyU=NULL;
	}
	...
	if(!RegQueryValueExA(hInstallerKey,"serversuite",NULL,&dwType,(LPBYTE)registration.data(),&dwLen) || dwType!=REG_SZ)
	{
		...
		if(!RegQueryValueExA(hInstallerKey,"server",NULL,&dwType,(LPBYTE)registration.data(),&dwLen) || dwType!=REG_SZ)
		{
			...
			{
				if (hInstallerKeyU==NULL)
					registration="";
				else
				{
					dwLen=registration.size();
					if(!RegQueryValueExA(hInstallerKeyU,"serversuite",NULL,&dwType,(LPBYTE)registration.data(),&dwLen) || dwType!=REG_SZ)
					{
						dwLen=registration.size();
						if(!RegQueryValueExA(hInstallerKeyU,"server",NULL,&dwType,(LPBYTE)registration.data(),&dwLen) || dwType!=REG_SZ)
						{
							dwLen=registration.size();
							if(!RegQueryValueExA(hInstallerKeyU,"servertrial",NULL,&dwType,(LPBYTE)registration.data(),&dwLen) || dwType!=REG_SZ)
								registration="";
							else
...
#else
	cvs::string os;
	cvs::sprintf(os,128,"%s-%s-%s", CVSNT_TARGET_VENDOR, CVSNT_TARGET_OS, CVSNT_TARGET_CPU);
	cvs::sprintf(url,128,"/cvspro/prods-pre.asp?register=stats&os=%s&sp=%s&ver=%s&avgtime=%u&up=%u&users=%u&uusers=%u&susers=%u&id=%s&cp=%u&major=%u&minor=%u",
		(const char *)os.c_str(),"none",CVSNT_PRODUCTVERSION_SHORT,0,0,0,0,0,"0000",0,0,0);

#endif
	return true;
}

static void serve_server_stats(char *arg)
{
	cvs::string url;

    if (SendStatistics(url))
	{
		...
		buf_output0(buf_to_net,url.c_str());
	}
...
#define REQ_LINE(n, f, s) {n, s}
  ...
  REQ_LINE("server-stats-enabled", serve_server_statsok, 0),
  REQ_LINE("server-statistics", serve_server_stats, 0),
  REQ_LINE("server-stats-sent", serve_server_statssent, 0),
...
finish_command:
	...
	TRACE(3,"proxy main - free_cvsroot_t");
	free_cvsroot_t(current_parsed_root);
	return exitstatus;
}
