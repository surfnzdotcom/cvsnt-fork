...
static int verify_access (PERMPROC permproc, const char *dir, const char *file, const char *update_dir, const char *tag, const char **message, const char **type_message)
{
    ...
    {
    	TRACE(3,"No point in verifying access if the repo is remote");
        return 1;
    }
    ...
    PERMPROC permproc)
{
	...
	TRACE(3,"start_recursion() dirleaveproc is %s",(frame.dirleaveproc != NULL)?"OK":"NULL");
    ...
    if (argc == 0)
    {
	TRACE(3,"start_recursion - argument count (argc) == 0");
	 ...
	 */
	TRACE(3,"* There were no arguments, so we'll probably just recurse. The");
	TRACE(3,"* exception to the rule is when we are called from a directory");
	TRACE(3,"* without any CVS administration files.  That has always meant to");
	TRACE(3,"* process each of the sub-directories, so we pretend like we were");
	TRACE(3,"* called with the list of sub-dirs of the current dir as args");
	if (just_subdirs)
	{
		TRACE(3,"start_recursion: just_subdirs");
		...
		TRACE(3,"* If there are no sub-directories, there is a certain logic in");
		TRACE(3,"  favor of doing nothing, but in fact probably the user is just");
		TRACE(3,"  confused about what directory they are in, or whether they");
		TRACE(3,"  cvs add'd a new directory.  In the case of at least one");
		TRACE(3,"  sub-directory, at least when we recurse into them we");
		TRACE(3,"  notice (hopefully) whether they are under CVS control.");
		...
		TRACE(3,"* In the the case \"cvs update foo bar baz\", a call to");
		TRACE(3,"  send_file_names in update.c will have sent the");
		TRACE(3,"  appropriate \"Argument\" commands to the server.  In");
		TRACE(3,"  this case, that won't have happened, so we need to");
		TRACE(3,"  do it here.  While this example uses \"update\", this");
		TRACE(3,"  generalizes to other commands.");
		...
		TRACE(3,"This is the same call to Find_Directories as above.");
		TRACE(3,"start_recursion: args_to_send_when_finished = Find_Directories()");
		args_to_send_when_finished = Find_Directories ((char *) NULL,
							       W_LOCAL,
							       (List *) NULL, NULL);
		if (args_to_send_when_finished!=NULL)
			TRACE(3,"start_recursion: args_to_send_when_finished!=NULL");
		else
			TRACE(3,"start_recursion: args_to_send_when_finished==NULL");
	    }
	}
	...
	{
		TRACE(3,"start_recursion: !just_subdirs - call addlist(%s, \".\")",(dirlist!=NULL)?"dirlist!=NULL":"dirlist==NULL");
	    addlist (&dirlist, ".");
		TRACE(3,"start_recursion: addlist() returns and now %s ",(dirlist!=NULL)?"dirlist!=NULL":"dirlist==NULL");
	}

	goto do_the_work;
    }
	else
		TRACE(3,"start_recursion - argument count (argc) == %d",argc);
     ...
     */
	TRACE(3,"* There were arguments, so we have to handle them by hand. To do");
	TRACE(3,"* that, we set up the filelist and dirlist with the arguments and");
	TRACE(3,"* call do_recursion.  do_recursion recognizes the fact that the");
	TRACE(3,"* lists are non-null when it starts and doesn't update them.");
	TRACE(3,"");
	TRACE(3,"* explicitly named directories are stored in dirlist.");
	TRACE(3,"* explicitly named files are stored in filelist.");
	TRACE(3,"* other possibility is named entities whicha are not currently in");
	TRACE(3,"* the working directory.");
    
    for (i = 0; i < argc; i++)
    {
	/* if this argument is a directory, then add it to the list of
	   directories. */
	TRACE(3,"* if this argument is a directory, then add it to the list of directories.");

	if (isdir (argv[i]))
	{
	    strip_trailing_slashes (argv[i]);
		TRACE(3,"start_recursion: isdir (%s) - call addlist(%s, \"%s\")",argv[i],argv[i],(dirlist!=NULL)?"dirlist!=NULL":"dirlist==NULL");
	    addlist (&dirlist, argv[i]);
		TRACE(3,"start_recursion: isdir/addlist() returns and now %s ",(dirlist!=NULL)?"dirlist!=NULL":"dirlist==NULL");
	}
	else
	{
	    /* otherwise, split argument into directory and component names. */
		TRACE(3,"* otherwise, split argument into directory and component names.");
	    char *dir;
	    char *comp;
	    char *file_to_try;
		...
		TRACE(3,"* Now break out argv[i] into directory part (DIR) and file part (COMP).");
		TRACE(3,"  DIR and COMP will each point to a newly malloc'd string. ");
	    dir = xstrdup (argv[i]);
		    ...
		    )
		{
		    ...
		    if (isdir (reposfile))
			{
			TRACE(3,"start_recursion: isdir|file(%s) - call addlist(%s, \"%s\")",reposfile,reposfile,(dirlist!=NULL)?"dirlist!=NULL":"dirlist==NULL");
			addlist (&dirlist, argv[i]);
			TRACE(3,"start_recursion: isdir|file/addlist() returns and now %s ",(dirlist!=NULL)?"dirlist!=NULL":"dirlist==NULL");
			}
		    else
			addfile (&files_by_dir, dir, comp);
	...
	TRACE(3,"* At this point we have looped over all named arguments and built");
	TRACE(3,"  a coupla lists.  Now we unroll the lists, setting up and");
	TRACE(3,"  calling do_recursion.");
    ...
    if (dirlist != NULL)
    {
	TRACE(3,"start_recursion: then do_recursion on the frame if dirlist != NULL.");
    do_the_work:
	TRACE(3,"start_recursion/do_the_work: do_recursion() on the frame (dirlist != NULL).");
	err += do_recursion (&frame, 1);
    }
	else
		TRACE(3,"start_recursion: cannot do_recursion on the dirlist.");
	
    /* Free the data which expand_wild allocated.  */
	TRACE(3,"start_recursion: Free the data which expand_wild allocated.");
    free_names (&argc, argv);

    xfree (update_dir);
	xfree (update_repos);

	TRACE(3,"start_recursion: (args_to_send_when_finished %s NULL)",(args_to_send_when_finished != NULL)?"!=":"==");
	...
	*/
	...
	TRACE(3,"start_recursion: Construct a fake argc/argv pair. ");
	...
	TRACE(3,"start_recursion: * We don't want to expand widcards, since we've just created a list of directories directly from the filesystem.");
	...
	TRACE(3,"start_recursion: return %d.",err);
    return (err);
}
...
static int do_recursion (struct recursion_frame *frame, int top_level)
{
	...
	TRACE(3,"do_recursion: Check the value in CVSADM_ROOT ");
    ...
    {
	TRACE(3,"do_recursion: server does not appear to be active");
	...
	TRACE(3,"do_recursion: Fill in repository with the current repository");
    /*
	...
	{
		TRACE(3,"(repository && !current_parsed_root->isremote)");
		mapped_repository = map_repository(repository);
	}
	else
	{
		TRACE(3,"!(repository && !current_parsed_root->isremote) - so just strdup");
     ...
     */
	TRACE(3,"* The filesdoneproc needs to be called for each directory where files");
	TRACE(3,"* processed, or each directory that is processed by a call where no");
	TRACE(3,"* directories were passed in.  In fact, the only time we don't want to");
	TRACE(3,"* call back the filesdoneproc is when we are processing directories that");
	TRACE(3,"* were passed in on the command line (or in the special case of `.' when");
	TRACE(3,"* we were called with no args");
	TRACE(3,"do_recursion: dirlist %s NULL && filelist == NULL",(dirlist != NULL)?"!=":"==",(filelist == NULL)?"==":"!=");
    if (dirlist != NULL && filelist == NULL)
		dodoneproc = 0;
	TRACE(3,"do_recursion: dodoneproc=%d",dodoneproc);

    /*
     * If filelist or dirlist is already set, we don't look again. Otherwise,
     * find the files and directories
     */
	TRACE(3,"* If filelist or dirlist is already set, we don't look again. Otherwise,");
	TRACE(3,"* find the files and directories");
    if (filelist == NULL && dirlist == NULL)
    {
	/* both lists were NULL, so start from scratch */
	TRACE(3,"* both lists were NULL, so start from scratch ");
	if (frame->fileproc != NULL && frame->flags != R_SKIP_FILES)
	{
	    int lwhich = frame->which;

		TRACE(3,"* In the !(which & W_LOCAL) case, we filled in repository");
		TRACE(3,"  earlier in the function.  In the (which & W_LOCAL) case,");
		TRACE(3,"  the Find_Names function is going to look through the");
		TRACE(3,"  Entries file.  If we do not have a repository, that");
		TRACE(3,"  does not make sense, so we insist upon having a");
		TRACE(3,"  repository at this point.  Name_Repository will give a");
		TRACE(3,"  reasonable error message. ");
	    ...
	    if (repository == NULL)
		{
			TRACE(3,"do_recursion: repository == NULL");
			repository = Name_Repository ((char *) NULL, update_dir);
		}
		else
			TRACE(3,"do_recursion: repository != NULL");
		...
		{
			TRACE(3,"do_recursion: mapped_repository == NULL");
			mapped_repository = map_repository(repository);
		}
		else
			TRACE(3,"do_recursion: mapped_repository != NULL");
		...
		TRACE(3,"do_recursion: find the files and fill in entries if appropriate");
	    if (process_this_directory)
	    {
			TRACE(3,"do_recursion: process_this_directory, call Find_Names()");
			filelist = Find_Names (mapped_repository, lwhich, frame->aflag,
						&entries, repository);
			if (filelist == NULL)
			{
				error (0, 0, "skipping directory %s", update_dir);
				TRACE(3,"* Note that Find_Directories and the filesdoneproc");
				TRACE(3,"  in particular would do bad things (\"? foo.c\" in");
				TRACE(3,"  the case of some filesdoneproc's).");
				...
				TRACE(3,"do_recursion: goto skip_directory");
				goto skip_directory;
			}
			TRACE(3,"do_recursion: Find_Names got a filelist");
	    }
	}
    ...
    else
    {
		...
		TRACE(3,"do_recursion: something was passed on the command line ");
		if (filelist != NULL && frame->fileproc != NULL)
		{
			/* we will process files, so pre-parse entries */
			TRACE(3,"do_recursion: we will process files, so pre-parse entries ");
			if (frame->which & W_LOCAL)
	...
	TRACE(3,"do_recursion: process the files (if any) ");
	...
	TRACE(3,"do_recursion: read lock it if necessary ");
	...
	/* For the server, we handle notifications in a completely different
	   ...
	   here.  */
	TRACE(3,"* For the server, we handle notifications in a completely different");
	TRACE(3,"  place (server_notify).  For local, we can't do them here--we don't");
	TRACE(3,"  have writelocks in place, and there is no way to get writelocks here");
	if (current_parsed_root->isremote)
	...
	/* process the files */
	TRACE(3,"do_recursion: process the files by calling walklist()");
	err += walklist (filelist, do_file_proc, &frfile);

	/* unlock it */
	TRACE(3,"do_recursion: unlock it %s",(should_readlock)?"by calling Lock_Cleanup_Directory()":"");
	if (should_readlock)
	    Lock_Cleanup_Directory();

	/* clean up */
	TRACE(3,"do_recursion: clean up by calling dellist(filelist)");
	if (filelist)
	dellist (&filelist);
    }

    /* call-back files done proc (if any) */
	TRACE(3,"do_recursion: call-back files done proc (if any) ");
    if (process_this_directory && dodoneproc && frame->filesdoneproc != NULL)
	{
		TRACE(3,"do_recursion: yes: call-back files done proc (if any) ");
		err = frame->filesdoneproc (frame->callerdat, err, (char*)mapped_repository,
				    (char*)(update_dir[0] ? update_dir : "."),
				    entries);
	}
	else
		TRACE(3,"do_recursion: no: don't call-back files done proc (%s%s%s) ",
					(process_this_directory)?"process_this_directory":"!process_this_directory",
					(dodoneproc)?"dodoneproc":"!dodoneproc",
					(frame->filesdoneproc != NULL)?"frame->filesdoneproc != NULL":"frame->filesdoneproc == NULL"
				);

 skip_directory:
	TRACE(3,"do_recursion: at skip_directory ");
	if(repository && !current_parsed_root->isremote)
	{
		TRACE(3,"(repository && !current_parsed_root->isremote)");
		fileattr_write ();
	...
	TRACE(3,"do_recursion: process the directories (if necessary) ");
    if (dirlist != NULL)
    {
		// BUGME!!!
	...
	TRACE(3,"do_recursion: really process the directories (if necessary) ");
	frent.frame = frame;
	frent.entries = entries;
	TRACE(3,"do_recursion() -- walklist (dirlist %s, do_dir_proc, (void *) &frent)",(dirlist==NULL)?"==NULL":"!=NULL");
	err += walklist (dirlist, do_dir_proc, (void *) &frent);
	TRACE(3,"do_recursion() -- walklist() returns and now %s ",(dirlist!=NULL)?"dirlist!=NULL":"dirlist==NULL");
    }
	else
		TRACE(3,"do_recursion: cannot process the directories (dirlist == NULL) ");
	if (dirlist)
    dellist (&dirlist);
	...
	TRACE(3,"do_recursion: free the saved copy of the pointer if necessary ");
	xfree (srepository);
...
static int do_file_proc (Node *p, void *closure)
{
	...
	TRACE(3,"do_file_proc: process the files \"%s\"",PATCH_NULL(p->key));
	if(!current_parsed_root->isremote)
	{
		TRACE(3,"(!current_parsed_root->isremote)");
		mapped_name = map_filename(finfo->virtual_repository, p->key, &mapped_file_repository);
	}
	else
	{
		TRACE(3,"(current_parsed_root->isremote) - so just strdup");
	...
	TRACE(3,"do_file_proc: mapped name of file \"%s\"",PATCH_NULL(mapped_name));
    ...
    {
	TRACE(3,"do_file_proc: frfile->frame->dosrcs && mapped_file_repository");
	...
	TRACE(3,"do_file_proc: call findnode_fn()");
	...
	if(p)
	{
		...
		TRACE(3,"do_file_proc: got a result from findnode_fn()");
		e = (Entnode *)p->data;
...
static int do_dir_proc (Node *p, void *closure)
{
	...
	else
	{
		...
		if(!current_parsed_root->isremote)
		{
			TRACE(3,"(!current_parsed_root->isremote) 2");
		...
		{
			TRACE(3,"(current_parsed_root->isremote) - so just xstrdup ");
    ...
    {
	TRACE(3,"do_dir_proc() -- set up update_dir - no dots ? ");
	if (update_dir[0] == '\0')
	    (void) strcpy (update_dir, dir);
	...
	TRACE(3,"do_dir_proc() -- frame->which & W_LOCAL ");
    ...
    if (!(frame->which&W_FAKE) && CVSroot_cmdline == NULL && !server_active)
    {
	TRACE(3,"do_dir_proc() -- !(frame->which&W_FAKE) && CVSroot_cmdline == NULL && !server_active ");
	...
	if(!current_parsed_root->isremote)
	{
		...
		TRACE(3,"do_dir_proc() -- (!current_parsed_root->isremote)");
		/* before we do anything else, see if we have any
		   per-directory tags */
		ParseTag_Dir (newrepos, &tag, &date, &nonbranch, NULL);
		TRACE(3,"do_dir_proc() -- verify_access ");
...
//				error(0,0,"%s",message);
	...
	if(dir_return!=R_SKIP_ALL)
	{
		TRACE(3,"do_dir_proc() -- dir_return!=R_SKIP_ALL");
    ...
    if(process_this_directory)
		TRACE(3,"do_dir_proc() -- process_this_directory");
	else
		TRACE(3,"do_dir_proc() -- !process_this_directory");

    if (dir_return == R_SKIP_ALL || dir_return == R_ERROR)
		TRACE(3,"do_dir_proc() -- dir_return == R_SKIP_ALL || dir_return == R_ERROR");
    else if(process_this_directory)
	{
		TRACE(3,"do_dir_proc() -- dir_return != R_SKIP_ALL && dir_return != R_ERROR && process_this_directory");
	...
	TRACE(3,"do_dir_proc() -- call-back dir entry proc (if any)");
    if (dir_return == R_SKIP_ALL || dir_return == R_ERROR)
		TRACE(3,"do_dir_proc() -- dir_return == R_SKIP_ALL || dir_return == R_ERROR");
	...
	/* If we're doing the actual processing, call direntproc.
           ...
           and recurse. FIXME. */
	TRACE(3,"do_dir_proc() -- If we're doing the actual processing, call direntproc.");
    ...
    if (dir_return != R_SKIP_ALL && dir_return !=R_ERROR)
    {
		TRACE(3,"do_dir_proc() -- only process the dir if the return code was 0.");
		...
		if (!(frame->which&(W_LOCAL|W_FAKE)))
		{
			TRACE(3,"do_dir_proc() -- !(frame->which&(W_LOCAL|W_FAKE))");
	...
	TRACE(1,"debug: 22");	
		...
		TRACE(3,"do_dir_proc() -- call-back dir leave proc (if any) %s%s",
			(process_this_directory)?"process_this_directory ":"",
			(frame->dirleaveproc != NULL)?"frame->dirleaveproc != NULL":"");
	...
	{
		TRACE(3,"do_dir_proc() -- cannot process the dir because the return code was %s.",(dir_return == R_SKIP_ALL)?"R_SKIP_ALL":"R_ERROR");
		if(directory_opened)
		{
			TRACE(3,"more efficient - close directory in do_dir_proc");
			close_directory();
		}
		fileattr_write();
		fileattr_free();
...
static int unroll_files_proc (Node *p, void *closure)
{
	...
	TRACE(3,"unroll_files_proc() begin");

    /* if this dir was also an explicitly named argument, then skip
       it.  We'll catch it later when we do dirs. */
	TRACE(3,"* if this dir was also an explicitly named argument, then skip");
	TRACE(3,"  it.  We'll catch it later when we do dirs.");
    n = findnode_fn (dirlist, dir);
    if (n != NULL)
	return (0);

    /* otherwise, call dorecusion for this list of files. */
	TRACE(3,"* otherwise, call dorecusion for this list of files.");
	...
	{
		int directory_opened_status=-1;
		TRACE(3,"* We want to open the directory anyway, so treat it as 'remote'.  This only affects rlog, etc. ");
		...
		*/
		TRACE(3,"call open_directory will all NULL");
	...
	TRACE(3,"more efficient - close directory in unroll_files_proc?");
	if(directory_opened)
		close_directory();
