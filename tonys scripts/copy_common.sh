...
if [ -e /cygdrive/d/SmartLoader/binRelease/smartloader.exe ]; then
  SMARTLOADER=/cygdrive/d/SmartLoader/binRelease/smartloader.exe
else
  SMARTLOADER=/cygdrive/d/cvsdeps/smartloader/smartloader.exe
fi
for i in "${TARGET_WIN32}"/*.exe
do 
	echo smartloader "$i"
	$SMARTLOADER "$i"
done
