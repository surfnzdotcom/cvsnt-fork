...
#define GETOSBUFSIZE 512
#define GETOSBUFSIZE2 2048
...
char *get_os_version()
{
	...
	LPVOID errbuf;
#endif
	TCHAR osvw[GETOSBUFSIZE2];
	static char osvc[GETOSBUFSIZE2];
	OSVERSIONINFOEX osex = {sizeof(OSVERSIONINFO)};
	SYSTEM_INFO sysinfo;
	PGNSI pGNSI;
	BOOL bOsVersionInfoEx;

	ZeroMemory(&osvw, sizeof(osvw));
	ZeroMemory(&osvc, sizeof(osvc));
	ZeroMemory(&osex, sizeof(OSVERSIONINFOEX));
	osex.dwOSVersionInfoSize = sizeof(OSVERSIONINFOEX);

	if( !(bOsVersionInfoEx = GetVersionEx ((OSVERSIONINFO *) &osex)) )
	{
	...
	switch (osex.dwPlatformId)
	{
      ...
      if ( osex.dwMajorVersion == 6 && osex.dwMinorVersion == 0 )
      {
         if( osex.wProductType == VER_NT_WORKSTATION )
             _tcscat (osvw, TEXT("Windows Vista "));
         else _tcscat (osvw, TEXT("Windows Server \"Longhorn\" ") );
      }

      if ( osex.dwMajorVersion == 5 && osex.dwMinorVersion == 2 )
      {
         if( GetSystemMetrics(SM_SERVERR2) )
            _tcscat(osvw, TEXT("Microsoft Windows Server 2003 \"R2\" "));
         else if( osex.wProductType == VER_NT_WORKSTATION &&
            sysinfo.wProcessorArchitecture==PROCESSOR_ARCHITECTURE_AMD64)
         {
            _tcscat(osvw, TEXT("Microsoft Windows XP Professional x64 Edition "));
         }
         else _tcscat (osvw, TEXT("Microsoft Windows Server 2003, "));
      }

      if ( osex.dwMajorVersion == 5 && osex.dwMinorVersion == 1 )
         _tcscat (osvw, TEXT("Microsoft Windows XP "));

      if ( osex.dwMajorVersion == 5 && osex.dwMinorVersion == 0 )
         _tcscat (osvw, TEXT("Microsoft Windows 2000 "));

      if ( osex.dwMajorVersion <= 4 )
         _tcscat (osvw, TEXT("Microsoft Windows NT "));
      ...
      if( bOsVersionInfoEx )
      {
         ...
         {
            if( osex.dwMajorVersion == 4 )
               _tcscat (osvw,  TEXT("Workstation 4.0 ") );
            else if( osex.wSuiteMask & VER_SUITE_PERSONAL )
               _tcscat (osvw,  TEXT("Home Edition ") );
            else _tcscat (osvw, TEXT("Professional ") );
         }
            
         // Test for the server type.
         else if ( osex.wProductType == VER_NT_SERVER || 
                   osex.wProductType == VER_NT_DOMAIN_CONTROLLER )
         {
            if(osex.dwMajorVersion==5 && osex.dwMinorVersion==2)
            {
               ...
               {
                   if( osex.wSuiteMask & VER_SUITE_DATACENTER )
                      _tcscat (osvw, TEXT("Datacenter Edition ")
                               TEXT("for Itanium-based Systems") );
                   else if( osex.wSuiteMask & VER_SUITE_ENTERPRISE )
                      _tcscat (osvw, TEXT("Enterprise Edition ")
                               TEXT("for Itanium-based Systems") );
               }

               else if ( sysinfo.wProcessorArchitecture ==
                         PROCESSOR_ARCHITECTURE_AMD64 )
               {
                   if( osex.wSuiteMask & VER_SUITE_DATACENTER )
                      _tcscat (osvw, TEXT("Datacenter x64 Edition ") );
                   else if( osex.wSuiteMask & VER_SUITE_ENTERPRISE )
                      _tcscat (osvw, TEXT("Enterprise x64 Edition ") );
                   else _tcscat(osvw, TEXT("Standard x64 Edition ") );
               }
               ...
               else
               {
                   if( osex.wSuiteMask & VER_SUITE_DATACENTER )
                      ...
                      _tcscat (osvw, TEXT("Enterprise Edition ") );
                   else if ( osex.wSuiteMask & VER_SUITE_BLADE )
                      _tcscat (osvw, TEXT("Web Edition ") );
                   else _tcscat (osvw, TEXT("Standard Edition ") );
               }
            }
            else if(osex.dwMajorVersion==5 && osex.dwMinorVersion==0)
            {
               if( osex.wSuiteMask & VER_SUITE_DATACENTER )
                  _tcscat (osvw, TEXT("Datacenter Server ") );
               else if( osex.wSuiteMask & VER_SUITE_ENTERPRISE )
                  _tcscat (osvw, TEXT("Advanced Server ") );
               else _tcscat (osvw, TEXT("Server ") );
            }
            else  // Windows NT 4.0 
            {
               if( osex.wSuiteMask & VER_SUITE_ENTERPRISE )
                  _tcscat (osvw, TEXT("Server 4.0, Enterprise Edition ") );
               else _tcscat (osvw, TEXT("Server 4.0 ") );
            }
         }
      }
      ...
      {
         HKEY hKey;
         TCHAR szProductType[GETOSBUFSIZE];
         DWORD dwBufLen=GETOSBUFSIZE*sizeof(TCHAR);
         LONG lRet;
         ...
         if( (lRet != ERROR_SUCCESS) ||
             (dwBufLen > GETOSBUFSIZE*sizeof(TCHAR)) )
            return FALSE;

         if ( lstrcmpi( TEXT("WINNT"), szProductType) == 0 )
            _tcscat(osvw, TEXT("Workstation ") );
         if ( lstrcmpi( TEXT("LANMANNT"), szProductType) == 0 )
            _tcscat(osvw, TEXT("Server ") );
         if ( lstrcmpi( TEXT("SERVERNT"), szProductType) == 0 )
            _tcscat(osvw, TEXT("Advanced Server ") );
         _stprintf(osvw+_tcsclen(osvw), TEXT("%d.%d "), osex.dwMajorVersion, osex.dwMinorVersion );
      }

      // Display service pack (if any) and build number.

      if( osex.dwMajorVersion == 4 && 
          lstrcmpi( osex.szCSDVersion, TEXT("Service Pack 6") ) == 0 )
      { 
         HKEY hKey;
         LONG lRet;
         ...
         if( lRet == ERROR_SUCCESS )
            _stprintf(osvw+_tcsclen(osvw), TEXT("Service Pack 6a (Build %d)"), 
            osex.dwBuildNumber & 0xFFFF );         
         else // Windows NT 4.0 prior to SP6a
         {
            _stprintf( osvw+_tcsclen(osvw), TEXT("%s (Build %d)"),
               osex.szCSDVersion,
               osex.dwBuildNumber & 0xFFFF);
         }
      ...
      if (osex.dwMajorVersion == 4 && osex.dwMinorVersion == 0)
      {
          printf ("Microsoft Windows 95 ");
          if (osex.szCSDVersion[1]=='C' || osex.szCSDVersion[1]=='B')
             _tcscat(osvw, TEXT("OSR2 ") );
      } 

      if (osex.dwMajorVersion == 4 && osex.dwMinorVersion == 10)
      {
          printf ("Microsoft Windows 98 ");
          if ( osex.szCSDVersion[1]=='A' || osex.szCSDVersion[1]=='B')
             _tcscat(osvw, TEXT("SE ") );
      } 

      if (osex.dwMajorVersion == 4 && osex.dwMinorVersion == 90)
      {
	...
	_stprintf(osvw+_tcsclen(osvw), TEXT(" [%d.%d.%d]"),osex.dwMajorVersion,osex.dwMinorVersion,osex.dwBuildNumber);
	if(osex.szCSDVersion[0])
		_stprintf(osvw+_tcsclen(osvw), TEXT(" (%s)"),osex.szCSDVersion);
#ifdef _MBCS 
	// this code should never be called since we either compile in ASCII for win9x or UNICODE for winnt
	// I just wrote this "in case"...
	wchar_t osvu[GETOSBUFSIZE2];
	// convert to unicode
	conv_res=MultiByteToWideChar(DEFAULT_CHARSET,MB_ERR_INVALID_CHARS,osvw,-1,osvu,sizeof(osvu));
	// convert unicode to ascii
	if (conv_res!=0)
		WideCharToMultiByte(CP_ACP, 0, osvu, -1, osvc, sizeof(osvc)*sizeof(char), NULL, NULL);
	else
	{
		dwLastErr=GetLastError();
		FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER|FORMAT_MESSAGE_FROM_SYSTEM,NULL,conv_res,MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),(LPTSTR)&errbuf,0,NULL);
		strcpy(osvc,(LPCSTR)errbuf);
		LocalFree( errbuf );
	}
#endif
#ifdef _UNICODE 
	// convert unicode to ascii
	conv_res=WideCharToMultiByte(CP_ACP, 0, osvw, -1, osvc, sizeof(osvc)*sizeof(char), NULL, NULL);
	if (conv_res==0)
	{
		dwLastErr=GetLastError();
		FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER|FORMAT_MESSAGE_FROM_SYSTEM,NULL,conv_res,MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),(LPSTR)&errbuf,0,NULL);
		strcpy(osvc,(LPCSTR)errbuf);
		LocalFree( errbuf );
	}
#endif
...
{
	CLibraryAccess::VerifyTrust(NULL,true);
	...
	else
	{
		...
		*g_defaultdomain='\0';
		...
		if(serv_active)
		{
		  if (!CGlobalSettings::GetGlobalValue("cvsnt","PServer","UnicodeServer",buffer,sizeof(buffer)))
		  {
			if(atoi(buffer))
		  ...
		  {
		  	// should be make the default CP_UTF8 
...
//		SetConsoleOutputCP(win32_global_codepage);
	...
	if(server_active)
		TRACE(3,"Server running on \"%s\"",get_os_version());
	else
		TRACE(3,"Client running on \"%s\"",get_os_version());
}
